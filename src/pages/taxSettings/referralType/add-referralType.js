...
import { postReferralType } from "src/apis/referralType";
...
`;
//=======================================================
export default function AddReferralType() {
  //=======================
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      referralTypeMutation.mutate(values);
    },
  });
  ...
  const referralTypeMutation = useMutation({
    ...
    onSuccess: (res) => {
      swal("Referral Type Added !", res.message, "success"),
  ...
  return (
    <>
      {/* ------------------------------ */}
      <Head>
        <title>Dashboard | Add-Referral Type</title>
      </Head>
      <Container
        sx={{
          padding: 5,
...
AddReferralType.getLayout = (page) => <DashboardLayout>{page}</DashboardLayout>;
