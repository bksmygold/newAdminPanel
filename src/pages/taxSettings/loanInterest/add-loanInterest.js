...
import { postLoanInterest } from "src/apis/loanInterest";
...
`;
//=======================================================
export default function AddLoanInterest() {
  //=======================
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      loanInterestMutation.mutate(values);
    },
  });
  ...
  const loanInterestMutation = useMutation({
    ...
    onSuccess: (res) => {
      swal("Loan Interest Added !", res.message, "success"),
  ...
  return (
    <>
      {/* ------------------------------ */}
      <Head>
        <title>Dashboard | Add-Loan Interest</title>
      </Head>
      <Container
        sx={{
          padding: 5,
...
AddLoanInterest.getLayout = (page) => <DashboardLayout>{page}</DashboardLayout>;
